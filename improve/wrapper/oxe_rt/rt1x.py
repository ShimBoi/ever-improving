...
class RT1Policy:
  ...
  def _small_action_filter_google_robot(self, raw_action: dict[str, np.ndarray | tf.Tensor], arm_movement: bool = False, gripper: bool = True) -> dict[str, np.ndarray | tf.Tensor]:
...
def main(cfg):
  ...
  lang_embed_model = hub.load("https://tfhub.dev/google/universal-sentence-encoder-large/5")
  ...
  processed_obs = {
        "image": None,
        "natural_language_embedding": add_seq_dim_padding(lang_embed_model([cfg.env.foundation.task] * BATCH_SIZE).numpy()), 
    }
  
  # test if model outputs different outputs for different inputs
  for i in range(10):
    processed_obs["image"] = np.zeros((BATCH_SIZE, 15, 300, 300, 3))
    processed_obs["image"][:, :, :, :] = i * 10
    raw_action, action = policy.action(processed_obs)
    
    print("before concat", action)
    action = np.concatenate([action["world_vector"], action["rot_axangle"], action["gripper"]], axis=1)
    raw_action = np.concatenate([raw_action["world_vector"], raw_action["rotation_delta"], raw_action["gripper_closedness_action"]], axis=1)
    ...
    print("Not loaded properly", np.all(variables == policy.variables))
    quit()
